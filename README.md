...
* `Salon-v0.2.5-x86_64.flatpak` for most Intel and AMD PCs.
* `Salon-v0.2.5-aarch64.flatpak` for ARM64 computers.

Open the downloaded file with your software installer, or install it from a
terminal:

```sh
flatpak install --user ~/Downloads/Salon-v0.2.5-x86_64.flatpak
```
...
```sh
flatpak install --user --or-update ~/Downloads/Salon-v0.2.5-x86_64.flatpak
```
...
```sh
grep 'Salon-v0.2.5-x86_64.flatpak$' SHA256SUMS | sha256sum --check -
```
...
and the rest of its GNOME integration. Download `salon_0.2.5-1_all.deb` from
the [latest release][latest-release], then install it with APT so dependencies
are resolved automatically:

```sh
sudo apt install ~/Downloads/salon_0.2.5-1_all.deb
```
...
after the 0.2.5 changes have been committed and pushed:

```sh
git tag -a v0.2.5 -m "Salon 0.2.5"
