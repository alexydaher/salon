...
Two widgets rather than one, sitting in the two top corners of the screen.
**What the machine is** — the time in the display face with tabular figures
(so the layout doesn't twitch every minute as digit widths change), the date
beside it in the body face, and the network and battery glyphs — goes top
*left*. **What the user can do** — Search, All apps, Connect a phone,
Settings, Power — stays top *right*.

They were one right-aligned strip until 2026-08-23. Splitting them puts the
status where every television, phone and set-top box already puts it, and it
stops the one part of the bar that is *pressable* from being crowded by five
glyphs that are not. Two overlay children rather than one full-width bar on
purpose: a Gtk.Box spanning the screen would be picked for pointer events
across the whole top strip, and the tiles scroll under it.

Those five buttons are a **focusable row**, not just mouse targets. Pressing
UP from the first row of tiles moves the cursor up here, exactly as it does
on every ten-foot launcher; DOWN drops back into the tiles. Before that they
were reachable only through MENU, which is invisible: nothing on screen said
the button existed, and a user without a controller had nothing but two dim
icons to guess at.

The two status glyphs §6.9 asks for are **honestly absent** rather than
stubbed — the network glyph disappears if NetworkManager isn't there to ask,
and the battery one only ever appears on a machine that actually has a
battery, which the box under a television does not. Neither is focusable and
neither is clickable: they are the two things on this screen that report
rather than do, and Settings › Network is where the detail lives.

Everything in both widgets carries an accessible name. The clock's is the
time spoken in words rather than "14:05", and the five buttons are icon-only,
so without an explicit label a screen reader would announce them as "button".
"""
