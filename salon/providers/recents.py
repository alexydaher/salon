...
provider instead of after it: a tile can only be recent if it was launched,
and it could only be launched if it's in the catalogue on disk.
"""
...
class RecentsProvider(Provider):
    ...
    def rows(self, context: ProviderContext) -> list[Row]:
        ...
        tiles = [
            tiles_by_id[i] for i in self._settings.get_strv(_SETTINGS_KEY) if i in tiles_by_id
        ]
