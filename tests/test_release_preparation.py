...
def test_prepared_manifest_pins_the_release_commit(tmp_path: Path) -> None:
    ...
    result = run_script(
        "--tag", "v0.2.6", "--commit", commit, "--output", str(output)
    )
    assert result.returncode == 0, result.stderr
